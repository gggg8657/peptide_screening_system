...
Gate criteria (from gate_thresholds.yaml):
    ddG <= 498.4713 REU    (rosetta_ddg_max; SST14 FlexPepDock mean × 0.9)
...
from pipeline_local.schemas.io_schemas import RosettaResult, Step06Output, DockingResult
...
_DEFAULT_DDG_MAX: float = 498.4713
...
def apply_rosetta_gate(
    ...
    Args:
        ...
        ddg_threshold:  Maximum allowed ddG (default 498.4713 REU).
...
def _build_ref_paths() -> list:
    ...
    return [
        *([Path(env_override)] if env_override else []),
        ...
        repo_root / "AgenticAI4SCIENCE_pyrosetta_track" / "repos" / "ai4sci-kaeri" / "data" / "fold_test1_model_0.pdb",
        # stale path — PRST_N_FM 디렉토리 부재 시 skip (하위호환 유지)
...
def _get_reference_peptide_com() -> Optional[Tuple[float, float, float]]:
    ...
    Tier 3 fix (F11 / SOD 2026-05-12): stale PRST_N_FM path → 실 위치 추가.
    환경변수 REFERENCE_COMPLEX_PATH override 지원.
    """
    for ref_path in _build_ref_paths():
        if ref_path.exists():
    ...
    logger.warning(
        ...
        "Set REFERENCE_COMPLEX_PATH env var or place file at data/fold_test1/fold_test1_model_0.pdb."
    )
    return None


def _cif_to_pdb(cif_text: str) -> str:
    """mmCIF 텍스트를 PDB 텍스트로 변환한다.

    Tier 1 fix (F1 / Stage 9 postmortem CRIT-1): Boltz가 mmCIF 형식으로
    출력하지만 step05_docking이 변환 없이 .pdb 확장자로 저장. step06이
    PDB ATOM 형식으로 가정하여 peptide 정보 손실 발생. 본 변환으로 해결.

    Bio.PDB MMCIFParser + PDBIO를 사용. 실패 시 원본 텍스트 반환 (fail-soft).
    """
    try:
        from io import StringIO
        from Bio.PDB import MMCIFParser, PDBIO  # type: ignore[import-not-found]
        parser = MMCIFParser(QUIET=True)
        structure = parser.get_structure("converted", StringIO(cif_text))
        out = StringIO()
        io_writer = PDBIO()
        ...
        result = out.getvalue()
        if not result.strip():
            logger.warning("[Step06] mmCIF→PDB 변환 결과 빈 텍스트, 원본 반환")
            return cif_text
        return result
    except Exception as exc:
...
def _is_mmcif(text: str) -> bool:
    """텍스트가 mmCIF 형식인지 판별한다 (PDB 아닌 경우 cif 가정)."""
    head = text[:500].lstrip()
    return (
        head.startswith("data_")
        or "_atom_site." in text[:5000]
        or "_struct." in text[:2000]
    )
...
def _assemble_complex(receptor_pdb: str, peptide_pdb: str) -> str:
    ...
    Tier 1 fix (F1): peptide_pdb 또는 receptor_pdb가 mmCIF 형식이면
    PDB로 변환 후 처리. Boltz는 mmCIF 출력이라 변환 없이 ATOM 추출 시
    chain B 누락 → cache key 충돌 + peptide 정보 손실.
    """
    # F1 fix: mmCIF input detect + convert
    if _is_mmcif(peptide_pdb):
        logger.info("[Step06] peptide input이 mmCIF 형식 — PDB 변환 (F1 fix)")
        peptide_pdb = _cif_to_pdb(peptide_pdb)
    if _is_mmcif(receptor_pdb):
        logger.info("[Step06] receptor input이 mmCIF 형식 — PDB 변환 (F1 fix)")
        receptor_pdb = _cif_to_pdb(receptor_pdb)
...
def _get_reference_complex_path() -> Optional[str]:
    ...
    Tier 3 fix (F11 / SOD 2026-05-12): stale PRST_N_FM path → 실 위치 추가.
    환경변수 REFERENCE_COMPLEX_PATH override 지원.
    부재 시 None 반환 + warning (graceful fallback).
    """
    for p in _build_ref_paths():
        if p.exists():
            logger.info("[Step06] Reference complex found: %s", p)
            return str(p)
    logger.warning(
        "[Step06] Reference complex PDB not found in any known path. "
        "Set REFERENCE_COMPLEX_PATH env var or place file at data/fold_test1/fold_test1_model_0.pdb. "
        "Falling back to receptor COM + 15A offset."
    )
    return None
...
def _run_pyrosetta_subprocess(
    ...
    # Look for the script next to this module or in known repo locations
    # Tier 1 fix (F2 / Stage 9 postmortem): 실 위치 추가 — 디렉토리 리팩토링 후 stale path 정정
    script_candidates = [
        ...
        # Stage 9 dogfood에서 발견된 실 위치 (postmortem-R2R3-3path.md §3 CRIT-2)
        Path(__file__).parent.parent.parent / "AgenticAI4SCIENCE_pyrosetta_track" / "repos" / "ai4sci-kaeri" / "AG_src" / "scripts" / script_name,
        # 환경 변수 override 지원
        *([Path(os.environ["PYROSETTA_SCRIPTS_DIR"]) / script_name] if os.environ.get("PYROSETTA_SCRIPTS_DIR") else []),
    ]
...
def _stub_rosetta_result(complex_pdb: str) -> RosettaResult:
    """PyRosetta 미설치 시 반환하는 플레이스홀더 결과."""
    logger.warning("[Step06] Returning STUB Rosetta result for %s. ddg=0.0 is NOT a real calculation.", complex_pdb)
    return RosettaResult(
        seq_id="",
        ddg=0.0,
        total_score=0.0,
...
def enhanced_score(
    ...
    """
    try:
        from pipeline_local.pipeline.interface_analysis import per_residue_energy, compute_buried_surface_area
    except ImportError:
        raise ImportError(
            "interface_analysis module not available in pipeline_local. "
            "Ensure pipeline_local/pipeline/interface_analysis.py exists."
        )
...
def run_interface_analysis(complex_pdb: str) -> Dict[str, Any]:
    ...
    """
    try:
        from pipeline_local.pipeline.interface_analysis import analyze_interface
    except ImportError:
        raise ImportError(
            "interface_analysis module not available in pipeline_local."
        )
...
def run_structure_validation(
    ...
    """
    try:
        from pipeline_local.pipeline.structure_validation import validate_structure
    except ImportError:
        raise ImportError(
            "structure_validation module not available in pipeline_local."
        )
...
if __name__ == "__main__":
    ...
    cfg: Dict[str, Any] = {
        ...
        "gate_thresholds": {"rosetta_ddg_max": 498.4713, "rosetta_clash_max": 10},
