...
class RosettaResult:
    ...
    source: str = "silo_b"          # 후보 출처: "silo_a" | "silo_b" (듀얼 모드 태깅용)
...
# =============================================================================
# Step 05c - Boltz-2 Cross-Validation (Boltz-2 선택성 교차 검증)
# Boltz-2 iPTM 매트릭스 기반 SSTR2 선택성 교차 검증
# AlphaFoldDB MSA + self-only a3m 우회 조합 활용
# =============================================================================

@dataclass
class BoltzSelectivityResult:
    """단일 후보의 Boltz-2 selectivity 교차 검증 결과."""
    seq_id: str                             # 시퀀스 고유 ID
    sequence: str                           # 아미노산 시퀀스 (one-letter code)
    sstr2_iptm: float                       # SSTR2 iPTM 점수 (높을수록 강한 결합)
    offtarget_iptm: Dict[str, float]        # {receptor_name: iPTM} (SSTR1/3/4/5)
    selectivity_margin: float               # sstr2_iptm - max(offtarget_iptm)
    best_receptor: str                      # off-target 중 가장 강하게 결합하는 수용체
    tier: str                               # T0/T1/T2/T3 (margin 기반 분류)

    def to_dict(self) -> Dict[str, Any]:
        return asdict(self)

    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> "BoltzSelectivityResult":
        return cls(**d)


@dataclass
class Step05cOutput:
    """Step 05c 출력: Boltz-2 selectivity 교차 검증 결과 전체."""
    results: List[BoltzSelectivityResult]
    passed_candidates: List[BoltzSelectivityResult]  # T2 이상 (margin ≥ 0.0)
    n_total: int
    n_passed: int                           # T2 이상 통과 수

    def to_dict(self) -> Dict[str, Any]:
        return {
            "results": [r.to_dict() for r in self.results],
            "passed_candidates": [r.to_dict() for r in self.passed_candidates],
            "n_total": self.n_total,
            "n_passed": self.n_passed,
        }

    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> "Step05cOutput":
        return cls(
            results=[BoltzSelectivityResult.from_dict(r) for r in d["results"]],
            passed_candidates=[BoltzSelectivityResult.from_dict(r) for r in d["passed_candidates"]],
            n_total=d["n_total"],
            n_passed=d["n_passed"],
        )

    def get_tier(self, tier: str) -> List[BoltzSelectivityResult]:
        """특정 Tier 이상의 후보 반환."""
