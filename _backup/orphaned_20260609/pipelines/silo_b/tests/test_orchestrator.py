...
import yaml
from pipelines.silo_b.src.orchestrator import SiloBOrchestrator


def _orchestrator_config(path: Path) -> str:
    cfg = {
        "pipeline_name": "test_silo_b_orchestrator",
        "config_version": "test",
        "schema_version": 1,
        ...
        "scoring": {
            "primary": {
                "docking_delta_g": {"weight": 0.45, "goal": "minimize", "clip": {"min": -14.0, "max": 0.0}},
                "stability": {"weight": 0.2, "goal": "maximize", "source": "pRosetta", "clip": {"min": -12.0, "max": 8.0}},
            },
            "auxiliary": {
                ...
                "hil_confidence": {"weight": 0.1, "goal": "maximize", "source": "hil", "clip": {"min": 0.0, "max": 1.0}},
            },
            "penalties": {"hard_violation": 8.0, "soft_violation_per_rule": 0.5, "duplicate_penalty": 0.1},
            "normalization": {"method": "minmax", "epsilon": 1e-8},
        },
        "orchestration": {
            "batch_size": {"generation": 4, "docking": 4, "approach_a": 1},
            "adaptive_steps": 1,
...
def test_full_pipeline_smoke(tmp_path: Path) -> None:
    cfg_path = _orchestrator_config(tmp_path / "orchestrator.yaml")
    result = SiloBOrchestrator(cfg_path).run()

    assert result.total_generated == 4
    assert result.filtered_count > 0
    assert result.scored_count > 0
    assert len(result.top_candidates) >= 1
    assert result.manifest["stage_counts"]["generated"] == result.total_generated
