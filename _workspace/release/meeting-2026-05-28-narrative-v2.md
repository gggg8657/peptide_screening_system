...
`gate_thresholds.yaml`에는 `rosetta_ddg_max = 498.4713`이 반영되었다.
**산출물**
- main commit `8e7e1cc`.
...
PR #85의 3-Layer Ensemble은 이 병목을 다루기 위한 framework로 main에 반영되었다.
Layer 1은 휴리스틱 계층이다.
...
현재 검증 결과는 R²=-0.028, Spearman ρ=-0.119, MAE=33.12 h이다.
R²가 음수라는 것은 평균값 예측보다 낮은 설명력을 의미한다.
따라서 Layer 2는 현재 의사결정 모델이 아니다.
...
### 5.4 의사결정 루프 변화
3-Layer 적용 전에는 ΔG 또는 단일 ADMET 값이 후보 순위에 직접 들어갈 위험이 있었다.
3-Layer 적용 후에는 다음 루프가 강제된다.
...
이 루프는 후보 우선순위를 바꾼다.
PRST-001은 Tier S이므로 합성 우선 후보로 유지된다.
그러나 ADMET=1.00 또는 half-life heuristic score는 합성 진행의 확정 근거가 아니다.
PRST-002~004도 Tier B 후보로 남지만, 실측 패키지 없이 "백업 lead"로 승격할 수 없다.
...
### 5.6 파급효과
첫째, 합성 후보 선정의 근거가 ΔG 중심에서 다중 조건 중심으로 이동한다.
둘째, OOD 경고가 있는 후보도 폐기하지 않고 실측 패키지로 넘기는 경로가 생긴다.
셋째, 실패 결과가 다음 모델 학습 데이터가 된다.
넷째, 6월 회의부터는 "예측값 보고"가 아니라 "예측값과 실측값의 불일치 보고"가 중요해진다.
다섯째, 자체 ML 모델 학습의 필요성이 더 명확해진다. 다만 현재 R²=-0.028 상태에서는 자체 모델을 즉시 의사결정용으로 쓰지 않는다.
...
PR #85의 3-Layer Ensemble은 이 한계를 해결하지 않는다.
대신 한계를 노출하고, 단일 도구의 과신을 막고, wet-lab 실측을 의사결정 루프 안으로 넣는 framework이다.
