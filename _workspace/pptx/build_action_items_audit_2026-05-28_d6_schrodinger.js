// Action Items 9건 원본 요구 vs 실행 결과 비교 PPTX — D-2 Schrödinger 의제 추가본
// 21 슬라이드 — D-6 갱신본 + 5월 회의 추가 의제(슈뢰딩거 도입 검토)
...
pres.title = "Action Items 9건 — Audit · D-2 갱신 (Schrödinger 의제, 2026-05-28 회의 준비)";
...
function addFooter(s, p, total) {
  ...
  s.addText("Action Items Audit · Deck 2026-05-28 D-2 + Schrödinger 의제", { x: 0.4, y: 7.25, w: 10.1, h: 0.25, fontSize: 9, color: C.light, fontFace: FONT_B, valign: "middle" });
...
// ───────────────────────────────────────────────
// Slide 21 — 슈뢰딩거 도입 검토 제안
// ───────────────────────────────────────────────
{
  const s = pres.addSlide();
  addTitle(s, "추가 의제 — 슈뢰딩거 도입 검토 (6월 회의까지)", "Critical 의제 · KAERI 라이센스 미보유 확인 → 도입 검토 필요");

  s.addShape(pres.shapes.RECTANGLE, { x: 0.5, y: 1.42, w: 12.3, h: 0.42, fill: { color: C.neg }, line: { type: "none" } });
  s.addText(
    "사용자 확인: KAERI Schrödinger 라이센스 미보유. 비용·일정은 영업 연락 및 KAERI 행정 검토 후 확정 가능.",
    { x: 0.65, y: 1.45, w: 12.0, h: 0.36, fontSize: 10.5, bold: true, color: "FFFFFF", fontFace: FONT_B, valign: "middle" }
  );

  const tableA = [
    [{ text: "현 도구 한계", options: { bold: true, fill: { color: C.dark }, color: "FFFFFF" } },
      { text: "슈뢰딩거 모듈", options: { bold: true, fill: { color: C.dark }, color: "FFFFFF" } }],
  ...
  const tableB = [
    [{ text: "회의 단계", options: { bold: true, fill: { color: C.dark }, color: "FFFFFF" } },
      { text: "슈뢰딩거", options: { bold: true, fill: { color: C.dark }, color: "FFFFFF" } }],
    ["(1) Specificity", "Glide SP/XP"],
    ["(4) Lead Compound", "MM-GBSA (Prime)"],
    ["(5) AA Modification", "BioLuminate"],
    ["(6) RI 표지 후 MD", "Desmond + FEP+"],
    ["(7) 제형 안정성", "WaterMap"],
  ];
  s.addTable(tableB, {
    x: 8.0, y: 1.96, w: 4.8,
    colW: [2.35, 2.45],
    fontSize: 9.2, fontFace: FONT_B,
    border: { type: "solid", pt: 0.5, color: C.border },
    rowH: 0.35,
    valign: "middle",
  });

  s.addShape(pres.shapes.RECTANGLE, { x: 0.5, y: 4.45, w: 7.25, h: 1.62, fill: { color: C.light }, line: { color: C.border, width: 1 } });
  s.addShape(pres.shapes.RECTANGLE, { x: 0.5, y: 4.45, w: 0.08, h: 1.62, fill: { color: C.secondary }, line: { type: "none" } });
  s.addText("도입 검토 항목", { x: 0.68, y: 4.52, w: 6.85, h: 0.28, fontSize: 12.5, bold: true, color: C.secondary, fontFace: FONT_H });
  s.addText(
    "• 라이센스 모델: 학술 vs 상업, 노드 수, 모듈별 cost-benefit\n"
      + "• Schrödinger Korea 영업 연락: 사용자 책임\n"
      + "• KAERI 행정 절차: 외부 SW 도입 승인\n"
      + "• 학습 곡선: 1~3개월 권고\n"
      + "• A-07 GPU 견적과 연결: Desmond/FEP+ GPU 가속",
    { x: 0.68, y: 4.86, w: 6.85, h: 1.08, fontSize: 9.8, color: C.text, fontFace: FONT_B, valign: "top" }
  );

  s.addShape(pres.shapes.RECTANGLE, { x: 8.0, y: 4.45, w: 4.8, h: 1.62, fill: { color: C.dark }, line: { type: "none" } });
  s.addText("의사결정 요청", { x: 8.18, y: 4.52, w: 4.45, h: 0.3, fontSize: 12.5, bold: true, color: C.secondary, fontFace: FONT_H });
  s.addText(
    "• 6월 회의까지 도입 검토 진행 동의?\n"
      + "• 책임: KAERI 행정 + 사용자 + AI팀\n"
      + "• 산출: 견적, 모듈 범위, 일정, GPU 연계 검토",
    { x: 8.18, y: 4.92, w: 4.45, h: 0.95, fontSize: 10.2, color: "FFFFFF", fontFace: FONT_B, valign: "top" }
  );

  s.addShape(pres.shapes.RECTANGLE, { x: 0.5, y: 6.3, w: 12.3, h: 0.72, fill: { color: C.accent }, line: { type: "none" } });
  s.addText("권고: 5월 회의에서 의제로 승인 → 6월 회의까지 KAERI 행정·견적·모듈 범위 검토 결과 보고.", {
    x: 0.65, y: 6.34, w: 12.0, h: 0.64, fontSize: 12.2, bold: true, color: "FFFFFF", fontFace: FONT_B, valign: "middle", align: "center",
  });

  addFooter(s, 21, TOTAL);
}

pres.writeFile({ fileName: "PRST_N_FM_ActionItems_Audit_2026-05-28_d6_schrodinger.pptx" })
