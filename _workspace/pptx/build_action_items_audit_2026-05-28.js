...
// 18 슬라이드 — 4/6 회의 audit + PR #85 3-Layer Ensemble 갱신(슬라이드 13–18)
...
pres.title = "Action Items 9건 — 원본 요구 vs 실행 결과 비교 · Ensemble 갱신 (2026-05-28 Deck)";
...
function addFooter(s, p, total) {
  ...
  s.addText("Action Items Audit · 4/6 회의 → Deck 2026-05-28 (액션 스냅샷 5/19)", { x: 0.4, y: 7.25, w: 9.2, h: 0.25, fontSize: 9, color: C.light, fontFace: FONT_B, valign: "middle" });
...
// ───────────────────────────────────────────────
{
  ...
  s.addText("발표 · D-8", { x: 7.1, y: 4.55, w: 5.6, h: 0.26, fontSize: 10, color: C.light, fontFace: FONT_B });
  s.addText("2026-05-28 (목)", { x: 7.1, y: 4.78, w: 5.6, h: 0.45, fontSize: 22, bold: true, color: "FFFFFF", fontFace: FONT_H });
  s.addText("9 / 9 종료 · PR #85", { x: 7.1, y: 5.28, w: 5.6, h: 0.38, fontSize: 20, bold: true, color: "FFFFFF", fontFace: FONT_H });
  s.addText("Ensemble b2eb50a\n(A-08 당일 삭제 반영)", { x: 7.1, y: 5.68, w: 5.6, h: 0.7, fontSize: 11, color: C.light, fontFace: FONT_B });
...
{
  const s = pres.addSlide();
  addTitle(s, "3-Layer Ensemble 갱신 — PR #85", "commit b2eb50a · 문서 근거 2026-05-20");
  s.addText("다음 슬라이드(14–18)는 워크스페이스 검증 Markdown에 따라 사실만 기재 (할루시네이션 없음).", {
  ...
  const b13 = [
    "PRST pepADMET 재검증: 합성 의뢰서 0.10/0.12/0.20/0.25 ↔ 실측 binary_toxicity=1.00 (CRITICAL)",
    "Layer 1: PlifePred2 Halflife 열 = 확률; SST-14만 calibration_table(0.05 h)",
    "Layer 2: PEPlife2 GAT 회귀 — test R² = -0.028, Spearman ρ = -0.119 (P4, Hard Cutoff 부적합)",
    "Layer 3: ADMET-AI CPU 104×5 OK — H-06 외삽 가드, recommended_for_decision=False",
    "운영 원칙: 단독 Gate 결정 금지 · 다중 도구 견제 + wet-lab 병행",
  ];
  b13.forEach((t, i) => {
    s.addText(`• ${t}`, { x: 0.72, y: 2.15 + i * 0.88, w: 11.9, h: 0.82, fontSize: 12.5, color: C.text, fontFace: FONT_B, valign: "top" });
  });
...
{
  const s = pres.addSlide();
  addTitle(s, "PRST-001~004 pepADMET 재검증", "_workspace/pepadmet_local/PRST-candidates_revalidation_2026-05-20.md");
  ...
  s.addText("막대: 동일 스케일(0–1) 비교 · pepADMET 재추론(4건 available: true)", { x: 0.5, y: 3.6, w: 12.3, h: 0.28, fontSize: 10, color: C.textMute, fontFace: FONT_B });
  s.addChart(
    pres.ChartType.bar,
    [
    ...
    { x: 0.45, y: 3.9, w: 12.4, h: 2.45, barDir: "col", barGrouping: "clustered", chartColors: [C.secondary, C.neg], valAxisMaxVal: 1, showLegend: true, legendPos: "b" }
  );

  s.addShape(pres.shapes.RECTANGLE, { x: 0.5, y: 6.42, w: 12.3, h: 0.7, fill: { color: C.warn }, line: { type: "none" } });
  s.addText(
    "결론: 4건 모두 학습·레퍼런스 도메인 밖 외삽 가능 — pepADMET 독성 확률을 단독 신뢰하면 안 됨 (LOW). Hard Cutoff admet_tox_max=0.3 관점에서 문서 권고: 의뢰서/게이트 갱신 필요.",
    { x: 0.65, y: 6.45, w: 12.0, h: 0.64, fontSize: 10, bold: true, color: "FFFFFF", fontFace: FONT_B, valign: "middle" }
  );
...
{
  const s = pres.addSlide();
  addTitle(s, "Layer 2 — PEPlife2 GAT · P4 정직 한계", "_workspace/pepmsnd_local/training_2026-05-20.md");
  ...
  const lines = [
    ...
    "Hold-out test (JSON): R² = -0.02826440502223293, Spearman ρ = -0.1190545794647295 — 예측력 없음에 가깝다",
    "DGL/libnvrtc: GraphBolt 로드 실패 사례 → PyG SMILES 경로 사용; env 정비·재학습은 로드맵",
    "PRST-001~004 추론 ~1.83 h 수렴 + ENDPOINT_CONFIDENCE grade P4 — Hard Cutoff 직접 사용 금지",
  ];
  lines.forEach((t, i) => {
    s.addText(`• ${t}`, { x: 0.68, y: 1.62 + i * 0.86, w: 12.0, h: 0.8, fontSize: 11.5, color: C.text, fontFace: FONT_B, valign: "top" });
  });
  s.addShape(pres.shapes.RECTANGLE, { x: 0.5, y: 6.28, w: 12.3, h: 0.52, fill: { color: C.neg }, line: { type: "none" } });
  s.addText("운영: 보조 지표 전용 · pharmacology_guards ENDPOINT_CONFIDENCE P4 (Hard Cutoff 반영 금지)", {
...
// ───────────────────────────────────────────────
// Slide 18 — Layer 3 + 마무리 메시지
// ───────────────────────────────────────────────
{
  const s = pres.addSlide();
  addTitle(s, "Layer 3 — ADMET-AI · 외삽 가드", "_workspace/admet_ai_local/installation_test_2026-05-20.md");
  s.addShape(pres.shapes.RECTANGLE, { x: 0.5, y: 1.48, w: 12.3, h: 2.35, fill: { color: C.light }, line: { color: C.border, width: 1 } });
  s.addText(
    "• PRST-001~004 + Octreotide — 5 compound CPU 추론 성공 (표: 각 ok=True, endpoints=104)\n"
      + "• 104 endpoint × 5 = 520 스칼라 데이터 포인트 (문서 산술)\n"
      + "• 매 compound extrapolation_warning=True, recommended_for_decision=False (H-06)\n"
      + "• 학습: 사전학습 ADMET-AI 패키지만 사용 · DOTA conjugate 도메인 업그레이드 없음",
    { x: 0.65, y: 1.58, w: 12.0, h: 2.15, fontSize: 11.5, color: C.text, fontFace: FONT_B, valign: "top" }
  );
  ...
  s.addTable(t18, {
    x: 0.5, y: 4.0, w: 8.5,
    colW: [2.2, 1.2, 1.5, 3.6],
    fontSize: 10.5, fontFace: FONT_B,
    border: { type: "solid", pt: 0.5, color: C.border },
    rowH: 0.34,
    valign: "middle",
  });
  s.addText("GPU used: False (CPU). 모델 초기화 ~0.14 s — 문서.", { x: 9.15, y: 4.05, w: 3.65, h: 0.9, fontSize: 10.5, color: C.textMute, fontFace: FONT_B, valign: "top" });

  s.addShape(pres.shapes.RECTANGLE, { x: 0.5, y: 5.95, w: 12.3, h: 1.05, fill: { color: C.primary }, line: { type: "none" } });
  s.addText(
    "3-Layer Ensemble framework 완성. 시도한 도구·학습·외부 컨택 모두 정직히 보고. 단독 결정 X, 다중 도구 견제 + wet-lab 병행 권고.",
    { x: 0.65, y: 6.02, w: 12.0, h: 0.92, fontSize: 14, bold: true, color: "FFFFFF", fontFace: FONT_H, align: "center", valign: "middle" }
  );

  addFooter(s, 18, TOTAL);
}

pres.writeFile({ fileName: "PRST_N_FM_ActionItems_Audit_2026-05-28.pptx" })
