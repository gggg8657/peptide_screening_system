// Action Items 9건 원본 요구 vs 실행 결과 비교 PPTX — 2026-05-20 갱신
// 13 슬라이드 (05-19판 + SSTR4 시그니처 HOTFIX). 베이스: build_action_items_audit_2026-05-19.js
...
function addFooter(s, p, total) {
  ...
  s.addText("Action Items Audit · 4/6 회의 → 2026-05-20", { x: 0.4, y: 7.25, w: 8, h: 0.25, fontSize: 9, color: C.light, fontFace: FONT_B, valign: "middle" });
...
function aiSlide(id, name, originalReq, kpi, executed, deliverables, gap, status) {
  ...
  addTitle(s, `${id} — ${name}`, `회의 KAERI-AIRL-MOM-2026-003 (2026-04-06) → 5/20 실행 결과`);
  ...
  s.addText("실행 결과 (5/20)", { x: 7.1, y: 1.6, w: 5.6, h: 0.4, fontSize: 14, bold: true, color: C.accent, fontFace: FONT_H });
...
// ───────────────────────────────────────────────
{
  ...
  s.addText("2026-05-20", { x: 0.7, y: 5.8, w: 5.6, h: 0.5, fontSize: 24, bold: true, color: "FFFFFF", fontFace: FONT_H });
...
{
  const s = pres.addSlide();
  addTitle(s, "Audit 매트릭스 — 9건 종합", "회의 요구 충족도 + 갭 + 후속 작업 · 5/20 머지 PR #68 / #71 / #72 / #73 반영");
  ...
  const rows = [
    ...
    ["A-01", "SSTR1/3/4/5 위치 지정 도킹 좌표 + 정렬", { text: "✓ 달성", options: { color: C.pos, bold: true } }, "TM-align 미설치 → PyMOL cealign (RMSD 2.77-3.13Å 충족). 5/20: PR #72 SSTR4 시그니처 fix · PR #71 CID 정합 (+#70 무음 추정 차단) → 선택성 재산정 진행"],
    ...
    ["A-04", "복합 스코어링 + Tier 분류", { text: "✓ 달성", options: { color: C.pos, bold: true } }, "PR #62 + 5/20 PR #68: P1 sprint wrapper × composite 자동 enrichment (이월 과제 종료). Pareto+WSS+Tier S/A/B/FAIL, 테스트 통과"],
    ...
    ["A-09", "최종 후보 3-4개 + 합성 의뢰서", { text: "✓ 달성", options: { color: C.pos, bold: true } }, "Gate-2 진입 후보 4개 PRST-001~004 — 합성 경로 확정 runs_local/final_candidates/synthesis_orders/ (PR #73 문서 갱신)"],
...
// ───────────────────────────────────────────────
// Slide 3 — HOTFIX — SSTR4 시그니처 충돌 (PR #72)
// ───────────────────────────────────────────────
{
  const s = pres.addSlide();
  addTitle(s, "HOTFIX — SSTR4 시그니처 충돌 (PR #72)", "VILRYAKMKTA 공유 모티프 제거 · 오프타겟 선택성 오매칭 해소 · merged 2026-05-20");

  s.addShape(pres.shapes.RECTANGLE, { x: 0.5, y: 1.5, w: 6.0, h: 5.3, fill: { color: C.light }, line: { color: C.border, width: 1 } });
  s.addShape(pres.shapes.RECTANGLE, { x: 0.5, y: 1.5, w: 0.1, h: 5.3, fill: { color: C.neg }, line: { type: "none" } });
  s.addText("Root Cause", { x: 0.8, y: 1.6, w: 5.6, h: 0.4, fontSize: 14, bold: true, color: C.neg, fontFace: FONT_H });
  s.addText(
    "• _SSTR_SIGNATURES 에 VILRYAKMKTA 가 SSTR1·SSTR4 양쪽에 중복 등록\n• _match_sstr_signature() 가 dict 순서로 SSTR1 우선 검색 → SSTR4 PDB/CIF 가 SSTR1 로 오매칭\n• 오프타겟 selectivity 결과 오염 (PRST-001~004 경로 재확인 권고)",
    { x: 0.8, y: 2.2, w: 5.6, h: 2.85, fontSize: 11, color: C.text, fontFace: FONT_B, valign: "top" }
  );

  s.addShape(pres.shapes.RECTANGLE, { x: 6.8, y: 1.5, w: 6.0, h: 5.3, fill: { color: C.light }, line: { color: C.border, width: 1 } });
  s.addShape(pres.shapes.RECTANGLE, { x: 6.8, y: 1.5, w: 0.1, h: 5.3, fill: { color: C.accent }, line: { type: "none" } });
  s.addText("Fix / 검증 · 후속", { x: 7.1, y: 1.6, w: 5.6, h: 0.4, fontSize: 14, bold: true, color: C.accent, fontFace: FONT_H });
  s.addText(
    "머지: PR #72 (main)\n• 공유 모티프 제거 — 고유 서명만 유지\n  SSTR1: YSVVCLVGLCG / SSTR4: YALVCLVGLVG\n• pytest pipeline_local/tests/test_offtarget_dock_cif_chain.py → 24/24 PASS\n  (직전: SSTR4 케이스 FAIL 4건)\n 선택성 재산정(진행 표기):\n• PR #70 무음 추정 fallback 차단 가드 (#69 receptor 데이터 수정 선행 포함)\n• PR #71 G-1 candidate_id 포맷 정합 → 버킷 키 mismatch 제거 후 재실행 파이프라인",
    { x: 7.1, y: 2.15, w: 5.6, h: 4.65, fontSize: 11, color: C.text, fontFace: FONT_B, valign: "top" }
  );

  s.addShape(pres.shapes.RECTANGLE, { x: 0.5, y: 6.9, w: 12.3, h: 0.3, fill: { color: C.pos }, line: { type: "none" } });
  s.addText("상태: ✓ 패치 적용 완료  ·  후속: 기존/selectivity 결과 재산정 (진행)", { x: 0.7, y: 6.9, w: 12.0, h: 0.3, fontSize: 11, bold: true, color: "FFFFFF", fontFace: FONT_B, valign: "middle" });
...
aiSlide("A-01", "SSTR site-directed docking + 5종 정렬",
  ...
  "PR #61 머지 (5/19)\n• Center: (-5.595, -28.626, 52.210)\n• Radius: 13.0Å / Box: 26.1Å\n• 5종 정렬 (PyMOL cealign):\n  - SSTR1 RMSD 3.125Å\n  - SSTR3 RMSD 3.086Å\n  - SSTR4 RMSD 3.019Å\n  - SSTR5 RMSD 2.770Å\n• 모두 ≤4.0Å 충족\n• 5/20: PR #72 off-target 시그니처 수선 + PR #70·#71 가드/CID 적용 후 selectivity 재산정(진행)",
  "• extract_binding_pocket.py\n• align_subtypes.py\n• selectivity_runner.py (get_pocket_center, get_gnina_config)\n• 38/38 tests pass\n• PR #61 + PR #72 (+ #70·#71 selectivity rerun 선행 패치)",
  "TM-align 미설치로 PyMOL cealign 대체 (결과 동등). 기존 SSTR4 선택성 결과는 패치 적용 버전 기준 재확인",
  "✓ 달성");
...
aiSlide("A-04", "복합 스코어링 + Tier 분류",
  ...
  "PR #62 머지 — 6 신규 파일 + 5/20 PR #68 (P2 sprint × composite enrichment)\n• composite_scorer: --enrich-from-wrappers, D-AA UNAVAILABLE 신뢰도 가드\n• radiolysis_scorer.py (¹⁷⁷Lu)\n• 73+/73+ tests 패스 (증분 회귀)\n• Hard Cutoff 5게이트 유지:\n  G1 ΔG ≤ -95 / G2 sel ≥ 100× /\n  G3 radiolysis ≤ 3 / G4 tox ≤ 0.3 / G5 II < 40",
  "• pipeline_local/scoring/{composite,radiolysis}_scorer.py\n• pipeline_local/scripts/composite_scorer{_cli}.py\n• pipeline_local/tests/test_p1_sprint_wrapper_integration.py\n• PR #62 + PR #68",
  "P1 sprint wrapper × composite_scorer 자동 enrichment: PR #68 머지로 종료(Task #52 이월 항목 정리)",
  "✓ 달성");
...
aiSlide("A-07", "GPU 인프라 견적 수집",
  ...
  "△ 부분 달성");
addFooter(pres.slides[pres.slides.length - 1], 10, TOTAL);
...
aiSlide("A-09", "최종 후보 3-4개 + 합성 의뢰서",
  ...
  "• runs_local/final_candidates/synthesis_orders/PRST-001.md\n• runs_local/final_candidates/synthesis_orders/PRST-002.md\n• runs_local/final_candidates/synthesis_orders/PRST-003.md\n• runs_local/final_candidates/synthesis_orders/PRST-004.md\n• docs/meet_log/2026-04-06_action_items/ (00_MASTER_INDEX, A-04/A-05/A-09) — PR #73\n• PR #63",
  "다양성 86% (목표 80% 미달 — 14aa 구조 제약, A-09 정책상 WARN 후 진행). Gate-2 후보 4개 PRST-001~004 + 합성 의뢰서는 synthesis_orders/ 개별 md",
  "✓ 달성");
addFooter(pres.slides[pres.slides.length - 1], 12, TOTAL);
...
{
  const s = pres.addSlide();
  addTitle(s, "A-10 + Gate-2 진입 로드맵", "Gate-2 후보 4개 · 합성 의뢰서 runs_local/final_candidates/synthesis_orders/PRST-00x.md — PR #73 정정");
  ...
  const roadmap = [
    "0. selectivity 재산정: PR #72·#71·#70 반영 파이프라인으로 후보 키/서명 정합 (진행)",
    "1. Gate-2 후보 4개 합성 (Peptron, 4-6주) — 의뢰서 synthesis_orders/PRST-001~004.md",
...
pres.writeFile({ fileName: "PRST_N_FM_ActionItems_Audit_2026-05-20.pptx" })
