...
# Cursor Composer/채팅에 붙여넣기 좋은 레포 스냅샷을 stdout으로 출력합니다.
#
# 사용법:
#   ./scripts/cursor/repo-context-for-agent.sh
#   ./scripts/cursor/repo-context-for-agent.sh --json
# =============================================================================
...
if [[ "$JSON" -eq 1 ]]; then
    export _CURSOR_CTX_ROOT="$ROOT" _CURSOR_CTX_BRANCH="$branch" _CURSOR_CTX_DESCRIBE="$describe"
    python3 - <<'PY'
import json, os, subprocess

root = os.environ["_CURSOR_CTX_ROOT"]
branch = os.environ["_CURSOR_CTX_BRANCH"]
describe = os.environ["_CURSOR_CTX_DESCRIBE"]
