# 미팅 액션 대응 패키지 (ACTION_ITEMS_RESPONSE_REPORT)

**작성일**: 2026-04-03  
**액션·번호 단일 출처**: 저장소 루트 `meet_log_backup.md`  
**Silo B 기술 전문(액션 전문 없음)**: [`02_SILO_B_TECHNICAL_REPORT.md`](02_SILO_B_TECHNICAL_REPORT.md)  
...
*이미지는 보고서 루트 기준이 아니라 **파일 위치 기준 상대경로**로 본문에서 링크한다 (`00_main/`에서 `../screenshots/...` 또는 `../03_ui_walkthrough/...`).*
...
2차 회의(2026-03-09) 안건 1에서 RI팀이 정리한 Serum Stability 예측 도구 11개 + Gemini 추가 제안 7개를 **전수 조사**한 결과, 대부분 접속 불가·소분자 전용·기능 과장이었다. 아래 표는 각 도구의 실제 상태와 AI팀 구현체 대응 관계를 정리한 것이다.

#### 회의록 원본 추천 도구 (정확도 순 리스트)

| # | 도구 | 회의록 설명 | 실제 조사 결과 | 접속 | SST-14 적용 | AI팀 대응 (`pharma_properties.py`) |
|---|------|-----------|--------------|:----:|:-----------:|----------------------------------|
| 1 | **PepCalc** (Innovagen) | "혈장 반감기 예측 특화" | **설명 과장.** MW, pI, 전하, 흡광계수, 용해도 계산기. **반감기 예측 기능 없음.** Novo Nordisk 기반이라는 근거도 없음 (독립 회사) | ✅ | 물성만 | `calculate_mw()`, `calculate_pi()`, `calculate_net_charge()`, `calculate_extinction_coefficient()` — PepCalc 기능 완전 대체 |
| 2 | **Peptide2.0** | "Novo Nordisk 기반" | **합성 회사**. 계산 도구 아님. Novo Nordisk는 고객 예시일 뿐 | ✅ | 해당없음 | 합성 회사 → 파이프라인 scope 밖 |
| 3 | **PEPlife 2.0** (IIITD) | "반감기 예측" | **비교적 신뢰.** Raghava 그룹 실험 검증 half-life DB (r=0.743, 163 데이터). 예측기라기보다 DB+예측기 조합 | ✅ | 참고용 | `calculate_nend_rule_halflife()` — N-end rule 반감기 추정. cyclic SST-14 학습 데이터 부재로 외부 도구도 동일 한계 |
| 4 | **PlifePred** (IIITD) | "혈액/혈청 반감기 전용" | **비교적 신뢰.** 서열 기반 half-life 예측 서버 | ✅ | screening용 | 미통합 — cyclic 변형체 학습 데이터 부재로 예측값은 상대 순위 참고용 |
| 5 | **ExPASy PeptideCutter** | "프로테아제 절단 hotspot" | **신뢰.** 효소별 절단 위치 예측 기본 도구. 웹 전용 (API 없음) | ✅ | 가능 | `count_protease_sites()` — Chymotrypsin/Trypsin/NEP/Pepsin/Elastase/DPP-IV **6종 효소** 자체 구현. ExPASy보다 넓은 커버리지 |
| 6 | **ProteinSol** | "protease liability 특화" | **설명 부정확.** 이름대로 **solubility 예측** 도구 (맨체스터대). 프로테아제 무관 | ✅ | 간접적 | `calculate_gravy()` + `calculate_wimley_white()` — 소수성/막투과 지표로 용해도 간접 평가 |
| 7 | **DPPred** | "ExPASy PeptideCutter와 연동" | **실체 불분명.** 공식 서비스 확인 불가 | ❓ | 불명 | — |
| 8 | **PepAnalyzer** | "반감기 포함 15종, 공개 웹서버" | **논문 존재 (Amino Acids 2023), 서버 불안정.** URL 타임아웃 | ❌ | 불가 | `calculate_all()` 19개 항목으로 15종 property 대부분 커버 |
| 9 | **Stability** (BiRNN) | "비특이 상호작용 저항성, github.com/haddocking" | **실체 불확실.** 표 기재 조합이 일관되지 않음 | ❓ | 불명 | — |
| 10 | **ADMETlab 3.0** | "HLM/RLM 안정성 탭" | **실존, 소분자 중심.** SSL 만료, API 404. MW<500 학습 → SST-14 ~1600Da AD 밖 | ⚠️ | **부적합** | pepADMET으로 전환 (A-02 참조) |
| 11 | **SwissADME** | "logP, solubility 보조" | **실존.** 공식적으로 small molecules 대상. 펩타이드 반감기 예측 불가 | ✅ | **부적합** | GRAVY + MW/pI로 물성 초기 필터 자체 해결 |

#### Gemini 제안 도구

| # | 도구 | 실제 상태 | AI팀 대응 |
|---|------|----------|----------|
| 12 | **ProtParam** (ExPASy) | 신뢰. 기본 물성 계산기 | `calculate_instability_index()`, `calculate_aliphatic_index()`, `calculate_mw()`, `calculate_pi()` — **ProtParam 핵심 기능 전부 자체 구현** |
| 13 | **HSPred** | 접근성 불확실 | `calculate_nend_rule_halflife()` 로 대체 |
| 14 | **juliomarcopineda/peptide-serum-stability** (GitHub) | 존재 확인 필요 | cyclic SS bond 처리를 `calculate_pi(ss_bond_cysteines=)`, `calculate_mw(n_disulfide=)` 로 직접 구현 |
| 15 | **FrankWanger/ML_Peptide** (GitHub) | 위장관 기반, 혈청과 다른 환경 | protease site 카운트 + II로 간접 평가 |
| 16 | **CamSolPTM** | 접근성 불확실 | `calculate_gravy()` + `calculate_net_charge()` 로 용해도 경향 평가 |
| 17 | **Deep-SStable** (GitHub) | 접근성 불확실 | `count_protease_sites()` 6종 효소 + `calculate_instability_index()` 조합 |

#### 총평

회의 권장 도구 18개 중 **실제로 신뢰 가능한 것은 4~5개** (PEPlife 2.0, PlifePred, ExPASy PeptideCutter, ProtParam, SwissADME). 이 중에서도 **SST-14 cyclic 변형체에 특화된 도구는 없다.** 주요 문제:

1. **PepCalc = 반감기 예측기가 아님** — 회의록 1순위 추천의 핵심 전제가 틀림
2. **서로 다른 목적의 도구가 혼합** — 반감기 예측 vs 절단 위치 예측 vs 물성 계산은 완전히 다른 문제
3. **소분자 도구를 펩타이드에 적용 불가** — ADMETlab/SwissADME는 MW<500 학습, SST-14 ~1600Da

이에 따라 **문헌 기반 자체 구현** (`pharma_properties.py` 877줄, 19개 계산 함수)으로 전환했으며, `peptides` 패키지 GT 대비 78케이스 0오차를 달성했다. 특히 **SS bond Cys 이온화 제외 보정**은 어떤 외부 도구에서도 제공하지 않는 기능이다.

> 상세 비교: [부록 — 반감기 예측 구현 상세](../01_appendix/a01_halflife_and_protease_detail.md)
