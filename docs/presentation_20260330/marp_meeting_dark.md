...
style: |
  section {
    font-family: 'Noto Sans KR', 'SF Pro Display', sans-serif;
    font-size: 0.92rem;
    padding: 30px 42px;
    background: linear-gradient(160deg, #0d1117 0%, #161b22 100%);
  }
  section.cover {
    background: linear-gradient(135deg, #0d1117 0%, #1a1e2e 40%, #21262d 100%);
    display: flex; flex-direction: column; justify-content: center;
    text-align: center;
  }
  section.cover h1 { color: #58a6ff; font-size: 2.4rem; margin-bottom: 0.3rem; text-shadow: 0 0 30px rgba(88,166,255,0.3); }
  section.cover h2 { color: #8b949e; font-size: 1.2rem; font-weight: 300; letter-spacing: 0.05em; }
  section.cover p { color: #6e7681; font-size: 0.85rem; }
  section.divider {
    background: linear-gradient(135deg, #161b22 0%, #1c2333 50%, #0d1117 100%);
    display: flex; flex-direction: column; justify-content: center;
    text-align: center;
  }
  section.divider h1 { color: #f0883e; font-size: 2.0rem; border: none; text-shadow: 0 0 20px rgba(240,136,62,0.25); }
  section.divider h3 { color: #8b949e; font-weight: 300; font-size: 1.0rem; }
  h2 { color: #58a6ff; border-bottom: 2px solid #30363d; padding-bottom: 5px; margin-bottom: 12px; font-size: 1.2rem; }
  h3 { color: #f0883e; font-size: 0.95rem; margin-bottom: 6px; }
  table { font-size: 0.71rem; width: 100%; border-collapse: collapse; border: 1px solid #30363d; }
  th { background: #21262d; color: #58a6ff; padding: 5px 7px; border: 1px solid #30363d; font-weight: 600; }
  td { padding: 4px 7px; border: 1px solid #30363d; color: #c9d1d9; }
  tr:nth-child(even) td { background: rgba(22,27,34,0.6); }
  tr:hover td { background: rgba(88,166,255,0.08); }
  ul { margin: 3px 0; } ul li { margin-bottom: 2px; line-height: 1.4; font-size: 0.8rem; color: #c9d1d9; }
  strong { color: #e6edf3; }
  code { background: #30363d; color: #79c0ff; border-radius: 4px; padding: 1px 5px; font-size: 0.72rem; }
  blockquote { border-left: 3px solid #f0883e; background: rgba(240,136,62,0.08); padding: 0.25em 0.7em; font-size: 0.76rem; margin: 6px 0; color: #f0883e; }
  .req { background: rgba(56,139,253,0.1); border-left: 3px solid #388bfd; padding: 6px 10px; margin: 5px 0; font-size: 0.77rem; border-radius: 0 6px 6px 0; color: #79c0ff; }
  .res { background: rgba(63,185,80,0.1); border-left: 3px solid #3fb950; padding: 6px 10px; margin: 5px 0; font-size: 0.77rem; border-radius: 0 6px 6px 0; color: #7ee787; }
  .blk { background: rgba(210,153,34,0.1); border-left: 3px solid #d29922; padding: 6px 10px; margin: 5px 0; font-size: 0.77rem; border-radius: 0 6px 6px 0; color: #e3b341; }
  .fail { background: rgba(248,81,73,0.1); border-left: 3px solid #f85149; padding: 6px 10px; margin: 5px 0; font-size: 0.77rem; border-radius: 0 6px 6px 0; color: #ffa198; }
  .tag { display: inline-block; background: #30363d; color: #58a6ff; padding: 2px 8px; border-radius: 12px; font-size: 0.65rem; margin: 1px 2px; }
  ...
  footer { color: #484f58; font-size: 0.65rem; }
  a { color: #58a6ff; }
---
...
파이프라인은 5개 에이전트가 각 반복(iteration)마다 역할을 수행하며 협업합니다. Planner가 가설을 세우고, QC&Ranker가 걸러내고, Critic이 피드백하는 **과학적 방법론 루프**를 구현합니다.

| 에이전트 | 유형 | 역할 | 호출 시점 |
|---------|:----:|------|---------|
| **Planner** | LLM | 가설 생성, 돌연변이 전략, 파라미터 제안 | 반복 시작 |
| **QC & Ranker** | Code | 게이트 적용 (pLDDT, ddG, clash), 순위 매김 | Step04~06 후 |
| **DiversityMgr** | Code | 중복 서열 제거, 구조 다양성 확보 | 순위 후 |
| **Critic** | LLM | 실패 원인 분석, 다음 반복 파라미터 조정 제안 | 반복 종료 전 |
| **Reporter** | LLM | 시각화 생성, 순위표 작성, 반복 보고서 | 반복 종료 |

> 현재 Critic은 5/18 메트릭만 참조 중 → **19개 실패 유형으로 확장** 및 **긍정 피드백 루프**(성공 패턴 → Planner bias) 제안이 서버 발표자료에 포함되어 있습니다.
...
# Part 5 — A-04 상세
### A~E 클러스터 분류 — 약학적 목적별 계층 분류

---

## A-04: A~E 클러스터 — 각 클러스터의 약학적 목적

<div class="req">

**원본 지시**: "Critic Agent에 ClusterReport (A~E 분류) 기능을 추가할 것"
</div>

클러스터 A~E 설계의 핵심 원칙은 **"단일 최적 기준 없이, 목적에 따라 다른 최적 후보를 선별한다"**는 것입니다. 예를 들어 클러스터 A는 도킹 점수 최우선 후보를, 클러스터 D는 방사성의약품 합성에 유리한 물리화학적 특성을 가진 후보를 선별합니다. 이 계층 분류를 통해 Critic 에이전트는 각 클러스터에 맞는 다음 iteration 전략을 다르게 적용할 수 있습니다.

| 클러스터 | 유형 | 약학적 목적 | 선별 기준 (모두 AND) | 우선순위 |
|:--------:|------|------------|----------------|:----:|
| **A** | 결합 엘리트 | SSTR2 도킹 점수가 가장 우수한 후보 — 친화도 최우선 | ddG ≤ **-8.0** + clash ≤ **5.0** + pLDDT ≥ **75.0** + **FWKT 접촉** | 1 |
| **B** | 선택성 특화 | SSTR2 외 다른 SSTR 아형에 결합이 약한 후보 — 부작용 최소화 | selectivity_margin ≥ **3.0** + ddG < **-5.0** | 2 |
| **C** | 안정성 강화 | 혈중 안정성이 높고 프로테아제 분해에 저항성인 후보 — 반감기 연장 | Instability Index < **30.0** + BLOSUM62 ≥ **0** + protease sites ≤ **9** | 3 |
| **D** | 방사화학 최적 | ⁶⁸Ga/¹⁷⁷Lu 표지 및 정제에 유리한 물리화학적 특성 보유 | GRAVY ∈ **[-1.0, +0.5]** + \|charge\| ≤ **1.0** + n_strong ≥ **1** | 4 |
| **E** | 탐색 후보 | A~D 기준 미충족이지만 탐색 가치가 있는 후보 (폴백) | A~D 미충족 자동 배정 | 5 |

클러스터 우선순위가 1인 A가 가장 먼저 평가되며, A 조건을 충족하면 B~E는 평가하지 않습니다. 이 **계층적(hierarchical) 구조**는 Critic 에이전트가 "이 후보는 A이므로 결합 강화 방향으로 mutation을 지속"하는 식의 명확한 다음 행동 지침을 도출할 수 있게 합니다.

---

## A-04: `classify_cluster()` — Critic 에이전트 연동

`classify_cluster()` 함수는 단일 후보의 클러스터를 반환하고, `batch_classify()`는 전체 후보군에 대한 분류 결과와 클러스터별 통계를 동시에 반환합니다. Critic 에이전트는 클러스터 정보를 바탕으로 다음 mutation iteration의 방향을 동적으로 조정합니다. 예를 들어 `poor_selectivity` 피드백이 발생하면 SSTR2 결합부위의 hotspot constraint를 강화하고, 클러스터 B 기준을 목표로 재설계합니다.

### Critic 에이전트 피드백 루프

- **A 클러스터 달성 시**: 현재 mutation 방향 유지, FWKT 접촉 강화 집중
- **B 클러스터 미달 시**: `poor_selectivity` 피드백 → hotspot constraint 강화
- **C 클러스터 미달 시**: `instability_high` 피드백 → 불안정 잔기(D, E 등) 치환 제안
- **D 클러스터 미달 시**: `radio_chem_unfavorable` → GRAVY/charge 조정 mutation 제안
- **E 클러스터 배정 시**: 다양성 확보 목적으로 탐색 유지, 50 iteration 내 재분류 시도

<div class="res">

**검증**: `classify_cluster()` + `batch_classify()` → **57개 테스트 전체 통과** · `commit e5790dd` · 엣지 케이스 포함 (경계값, NaN, 음수 ddG 등)
</div>
...
# Part 9 — 로컬 모델 전환
### NIM Cloud API → On-Premise 3 conda env
